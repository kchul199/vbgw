...
class VoicebotAiEmulator(voicebot_pb2_grpc.VoicebotAiServiceServicer):
    ...
    async def StreamSession(self, request_iterator, context):
            ...
            try:
                async for chunk in request_iterator:
                    ...
                    if chunk.is_speaking and not is_barged_in:
                        print(f"🚨 [Emulator] Barge-in Detected! is_speaking=True")
                        is_barged_in = True
            except Exception as e:
        ...
        yield voicebot_pb2.AiResponse(
            ...
            text_content=f"Hello user from emulator."
        )
        ...
        for i in range(0, len(tts_audio), chunk_size):
            if is_barged_in:
                print(f"🛑 [Emulator] Halting TTS stream due to barge-in.")
                yield voicebot_pb2.AiResponse(
...
async def serve():
    ...
    voicebot_pb2_grpc.add_VoicebotAiServiceServicer_to_server(VoicebotAiEmulator("sample_tts.wav"), server)
    server.add_insecure_port('[::]:50051')
    print("=======================================")
    print("🚀 STT/TTS Mock Emulator Started!")
    print("Listening for Voicebot Gateway on :50051")
    print("=======================================")
    await server.start()
