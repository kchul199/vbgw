...
class VoicebotAiServiceServicer(voicebot_pb2_grpc.VoicebotAiServiceServicer):
    def ListenAndSpeak(self, request_iterator, context):
        logging.info("====================================")
        ...
        try:
            for req in request_iterator:
                ...
                # 사용자가 침묵(Silence) 중이라면?
                    ...
                    if is_user_speaking:
                        logging.info(f"🤐 [STT/VAD] User stopped speaking. (VAD Off, Gathered {speaking_duration} bytes of voice)")
                        is_user_speaking = False
                        ...
                        yield voicebot_pb2.AiResponse(
                            ...
                            text="네, 고객님 요청하신 사항을 처리하겠습니다. 삐 소리를 들려드릴게요."
                        )
                        
                        yield voicebot_pb2.AiResponse(
                            type=voicebot_pb2.AiResponse.TTS_TEXT,
                            text="네, 알겠습니다."
                        )
